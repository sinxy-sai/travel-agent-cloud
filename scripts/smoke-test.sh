...
HAS_AGENT_ENGINE_FIELD="$(printf '%s' "${HEALTH_JSON}" | python3 -c 'import json, sys; print("yes" if "agentEngine" in json.load(sys.stdin) else "no")')"
if [ "${HAS_AGENT_ENGINE_FIELD}" != "yes" ]; then
...
HAS_TRIP_PLAN_CONTEXT_NODE="$(printf '%s' "${TRIP_PLAN_AGENT_STATUS_JSON}" | python3 -c 'import json, sys; data=json.load(sys.stdin); events=(data.get("lastRunTrace") or {}).get("nodeEvents") or []; print("yes" if any(event.get("nodeName") in {"trip_context", "request_context"} and event.get("status") == "SUCCEEDED" for event in events) else "no")')"
if [ "${HAS_TRIP_PLAN_CONTEXT_NODE}" != "yes" ]; then
