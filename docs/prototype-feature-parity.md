...
| --- | --- |
| 景点、天气、酒店、规划等 `SimpleAgent` | LangGraph 节点：`research_agent` 汇总景点/天气/酒店/餐食事实，`route_budget` 汇总路线/预算，`planner_agent` 生成结构化行程 |
...
| Agent 规划 | 多 Agent：景点、天气、酒店、最终规划 | 已收敛为 `research_agent`、`route_budget`、`planner_agent`，并通过 FastMCP 工具支持 |
