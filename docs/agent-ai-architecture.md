...
第一版 `KnowledgeRetriever` 位于 `agent-runtime/app/knowledge.py`，目前使用确定性本地知识命中，目的是先固定 `retrieve_knowledge` 工作流边界。后续可以把实现替换为 PostgreSQL 关键词检索或 pgvector，但对 LangGraph 节点和 PlanningContext 注入契约保持不变。
