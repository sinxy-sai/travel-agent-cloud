...
当前最小 eval harness 位于 `scripts/ai-eval.py`。它使用离线 `MockTravelToolProvider` 运行固定案例，输出 JSON 报告，用于快速验证 schema、天数、路线、预算、trace 和质量分数。该脚本不是 smoke test，也不依赖真实 LLM 或高德服务。
