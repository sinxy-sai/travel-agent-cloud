...
- `travel-trip`：当前已开始实迁行程持久化能力。匿名本地用户的行程列表、详情、编辑、版本、恢复、删除和 Markdown 导出由它直接访问 PostgreSQL 处理；生成、AI 修订和登录用户路径暂时仍代理到 `agent-runtime`。
...
5. 将 `travel-trip` 从代理门面升级为真正拥有行程、版本和导出元数据的服务。当前已完成匿名用户读写路径的第一步实迁。
