...
Docker Compose 会启动 PostgreSQL、RabbitMQ、Redis、MinIO、`travel-mcp`、`travel-trip`、`travel-gateway`、`agent-runtime` 和前端。
...
- K3s 默认部署 frontend、travel-gateway、travel-trip、agent-runtime、agent-runtime-worker、travel-mcp、PostgreSQL、RabbitMQ、Redis、MinIO 和 Ingress。
...
```text
本地 Docker Compose:
localhost:5173 -> frontend nginx -> travel-gateway -> travel-trip / agent-runtime / travel-mcp

VPS K3s:
公网 IP 或域名 -> Ingress -> frontend / travel-gateway -> travel-trip / agent-runtime / travel-mcp
```
