...
- `travel-auth`：认证和用户领域服务。当前已直接处理注册、登录、退出登录、当前用户、密码修改、session 管理，以及登录用户和匿名本地用户 profile 读写；OAuth、邮箱验证、密码重置、账号删除和账户数据导入导出仍代理到 `agent-runtime`。
...
4. 继续迁移 `travel-auth` 的邮箱验证、密码重置、OAuth identity 和账户数据导入导出边界。
